...
impl BranchDiff {
    ...
    pub fn load_buffers(&mut self, cx: &mut Context<Self>) -> Vec<DiffBuffer> {
        ...
        self.project.update(cx, |_project, cx| {
            ...
            for item in repo.read(cx).cached_status() {
                ...
                let task =
                    Self::load_buffer(diff_base, branch_diff, project_path, repo.clone(), cx);
    ...
    fn load_buffer(
        ...
        match diff_base {
            DiffBase::Unstaged => cx.spawn(async move |project, cx| {
                let buffer = project
                ...
                let conflict_set = project
                    ...
                    .await;
                Ok((buffer, diff, conflict_set))
            }),
            DiffBase::Staged => cx.spawn(async move |project, cx| {
                let buffer = project
                    .update(cx, |project, cx| project.open_buffer(project_path, cx))?
                    .await?;
                ...
                let conflict_set = project
                    .update(cx, |project, cx| {
                        project.git_store().update(cx, |git_store, cx| {
                            git_store.open_conflict_set(index_buffer.clone(), cx)
                        })
                    })?
                    .await;
                Ok((index_buffer, diff, conflict_set))
            }),
            _ => cx.spawn(async move |project, cx| {
                let buffer = project
                    .update(cx, |project, cx| project.open_buffer(project_path, cx))?
                    .await?;
